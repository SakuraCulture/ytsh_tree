...
public class WebFrameworkUtils {
    ...
    private static WebProperties getProperties() {
        if (properties == null) {
            properties = SpringUtils.getBean(WebProperties.class);
        }
        return properties;
    }
    ...
    public static Integer getLoginUserType(HttpServletRequest request) {
        ...
        // 2. 其次，基于 URL 前缀的约定
        if (request.getServletPath().startsWith(getProperties().getAdminApi().getPrefix())) {
            return UserTypeEnum.ADMIN.getValue();
        }
        if (request.getServletPath().startsWith(getProperties().getAppApi().getPrefix())) {
