...
public class UserController {
    ...
    public CommonResult<PageResult<UserRespVO>> getUserPage(@Valid UserPageReqVO pageReqVO) {
        // 调试日志：打印请求参数
        log.info("[getUserPage] 请求参数: deptId={}, username={}, status={}",
                pageReqVO.getDeptId(), pageReqVO.getUsername(), pageReqVO.getStatus());

        // 获得用户分页列表
        PageResult<AdminUserDO> pageResult = userService.getUserPage(pageReqVO);

        // 调试日志：打印查询结果
        log.info("[getUserPage] 查询结果: total={}, list.size={}, first5Ids={}",
                pageResult.getTotal(), pageResult.getList().size(),
                pageResult.getList().stream().limit(5).map(AdminUserDO::getId).toList());
