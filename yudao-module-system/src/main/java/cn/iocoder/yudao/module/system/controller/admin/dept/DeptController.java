...
public class DeptController {
    ...
    public CommonResult<List<DeptRespVO>> getDeptList(DeptListReqVO reqVO) {
        // 调试日志
        log.info("[getDeptList] 请求参数: name={}, status={}", reqVO.getName(), reqVO.getStatus());

        List<DeptDO> list = deptService.getDeptList(reqVO);

        // 调试日志
        log.info("[getDeptList] 查询结果: total={}, first10Ids={}",
                list.size(), list.stream().limit(10).map(DeptDO::getId).toList());
