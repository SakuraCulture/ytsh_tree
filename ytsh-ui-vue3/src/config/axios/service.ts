...
service.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    if (tenantEnable && tenantEnable === 'true') {
      ...
      // console.log('[前端] 发送请求, URL:', config.url, ', tenant-id:', tenantId) // 调试日志
    ...
    // 打印 ELE 相关接口的完整请求数据
    // if (config.url && config.url.includes('/ele/')) {
    //   console.log('[ELE完整请求] URL:', config.url)
    //   console.log('[ELE完整请求] params:', JSON.stringify(config.params, null, 2))
    //   console.log('[ELE完整请求] data:', JSON.stringify(config.data, null, 2))
    // }
...
service.interceptors.response.use(
  async (response: AxiosResponse<any>) => {
    ...
    // 打印 ELE 相关接口的完整响应数据
    // if (config.url && config.url.includes('/ele/')) {
    //   console.log('[ELE完整响应] URL:', config.url)
    //   console.log('[ELE完整响应] data:', JSON.stringify(data, null, 2))
    // }
