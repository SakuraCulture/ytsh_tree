...
const resolveViewModule = (viewPath?: string) => {
  if (!viewPath) {
    return undefined
  }
  const normalizedPath = viewPath.replace(/^\//, '').replace(/\.(vue|tsx)$/, '')
  const candidatePaths = [
    `../views/${normalizedPath}.vue`,
    `../views/${normalizedPath}.tsx`
  ]
  if (!normalizedPath.endsWith('/index')) {
    candidatePaths.push(`../views/${normalizedPath}/index.vue`)
    candidatePaths.push(`../views/${normalizedPath}/index.tsx`)
  }
  for (const candidatePath of candidatePaths) {
    const component = modules[candidatePath]
    if (component) {
      return component
    }
  }
...
// 后端控制路由生成
const normalizeTopLevelPath = (route: AppCustomRouteRecordRaw) => {
  if (route.parentId === 0 && !isUrl(route.path) && !route.path.startsWith('/')) {
...
export const generateRoute = (routes: AppCustomRouteRecordRaw[]): AppRouteRecordRaw[] => {
  ...
  for (const route of routes) {
    ...
    if (!route.children && route.parentId == 0 && route.component) {
      ...
      const component = resolveViewModule(route?.component || route.path)
      ...
      if (route.children?.length) {
        ...
        data.component = resolveViewModule(route?.component || route.path)
      }
      if (route.children) {
