...
// toolHeader 显示
const toolHeader = computed(() => appStore.getToolHeader)

// 计算内容区域顶部偏移
const contentMarginTop = computed(() => {
  const tagsViewHeight = tagsView.value ? 'var(--tags-view-height)' : '0px'
  const toolHeaderHeight = toolHeader.value ? 'var(--top-tool-height)' : '0px'
  return `calc(${toolHeaderHeight} + ${tagsViewHeight})`
})

const contentMarginTopStyle = computed(() => {
  if (!fixedHeader.value) return ''
  const tags = tagsView.value ? 'var(--tags-view-height)' : '0px'
  const tool = toolHeader.value ? 'var(--top-tool-height)' : '0px'
  return `calc(${tool} + ${tags})`
})
      ...
      <>
        ...
        >
          <ElScrollbar
            ...
            style={fixedHeader.value ? { marginTop: contentMarginTopStyle.value, height: `calc(100% - ${contentMarginTopStyle.value})` } : undefined}
          >
            <div
              class={[
                {
    ...
    return (
      <>
        <div class="relative flex items-center bg-[var(--top-header-bg-color)] layout-border__bottom dark:bg-[var(--el-bg-color)]">
          ...
          {toolHeader.value ? <ToolHeader class="flex-1"></ToolHeader> : undefined}
        </div>
        <div class="absolute left-0 top-[var(--logo-height)] h-[calc(100%-var(--logo-height))] w-full flex">
          ...
          >
            <ElScrollbar
              v-loading={pageLoading.value}
              class={`${prefixCls}-content-scrollbar`}
              style={(fixedHeader.value && tagsView.value) ? { marginTop: tagsView.value ? 'var(--tags-view-height)' : '0px', height: 'calc(100% - var(--tags-view-height))' } : undefined}
            >
              {tagsView.value ? (
                <TagsView
                  class={[
    ...
    return (
      <>
        ...
        >
          ...
          {toolHeader.value ? <ToolHeader></ToolHeader> : undefined}
        </div>
        <div class={[`${prefixCls}-content`, 'w-full h-[calc(100%-var(--top-tool-height))]']}>
          <ElScrollbar
            v-loading={pageLoading.value}
            class={`${prefixCls}-content-scrollbar`}
            style={(fixedHeader.value && tagsView.value) ? { marginTop: 'var(--tags-view-height)', height: 'calc(100% - var(--tags-view-height))' } : undefined}
          >
            {tagsView.value ? (
              <TagsView
                class={[
